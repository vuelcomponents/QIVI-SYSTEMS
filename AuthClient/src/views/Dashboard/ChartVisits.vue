<template>
  ...
  >
    <p>{{ $t('monthlyActivities') }}</p>
  </div>
  <div
    class="card flex h-full w-full flex-1 items-center justify-center rounded-b-lg bg-trueGreen-400/5 p-4 ring-1 ring-surface-200 dark:bg-black/0 dark:ring-surface-700"
  >
    <Chart
      type="bar"
      :data="chartData"
...
const props = defineProps<{
  labels: Array<string>;
  monthlyActivities: Array<number>;
}>();
...
const setChartData = () => {
  return {
    ...
    datasets: [
      {
        label: t('monthlyActivities'),
        data: props.monthlyActivities,
        fill: false,
        tension: 0.4,
